...
from models.conv2d import ConvNeuralNet
...
def train_model(rank, args):
    ...
    # DPP initialization
    print(f"Running Distributed ConvNet on rank {rank}.")
    ...
    ##-------------------------------------------------------
    model = ConvNeuralNet(len(ciphar_classes)).to(rank)
    ...
    if rank == 0:
        ...
        if not os.path.exists('./trained'):
            os.mkdir('./trained')
        torch.save(state, './trained/ciphar10-conv2d.pth')
...
def run_train_model(training_func, world_size):
  ...
  parser = argparse.ArgumentParser("PyTorch - Training CONV2D on CIFAR10 Dataset")
